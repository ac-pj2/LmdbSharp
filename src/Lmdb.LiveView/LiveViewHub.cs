...
public sealed class LiveViewHub
{
    ...
    /// <summary>Open an in-process connection — the same session lifecycle as a
    /// WebSocket connection, with the transport pumped by the caller (a WebView
    /// bridge, an embedded host, a test harness). <paramref name="configure"/>
    /// runs before Mount; the initial render is queued on the connection's
    /// outbound stream before this returns. Dispose the connection to tear the
    /// session down.</summary>
    public LiveViewConnection Connect(string viewName, Action<DeltaLiveView>? configure = null)
    {
        var sessionId = Guid.NewGuid().ToString("N");
        var view = _factory(viewName);
        view.SessionId = sessionId;
        ...
        return new LiveViewConnection(this, view);
    }

    internal void Teardown(DeltaLiveView view)
    {
    ...
    internal static void EnqueueEvent(ReadOnlySpan<byte> json, DeltaLiveView view)
    {
        try
        {
